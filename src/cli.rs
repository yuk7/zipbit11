...
pub fn run_from_env() {
    ...
    match parsed {
        ParsedArgs::Help => print_help(),
        ParsedArgs::Run {
            mode,
            file_path,
            selection,
        } => {
            if let Err(e) = zip::process(file_path, mode, selection) {
                eprintln!("Error: {e}");
                process::exit(1);
            }
        }
...
fn parse_args(args: &[String]) -> Result<ParsedArgs<'_>, String> {
    match args.len() {
        ...
        4 => {
            ...
            match mode {
                Mode::Detail | Mode::Set | Mode::Clear | Mode::Toggle => Ok(ParsedArgs::Run {
                    mode,
                    file_path,
                    selection: Some(selection),
                }),
                Mode::Status => Err(format!("'{}' does not take an entry selector", command)),
            }
        }
...
fn parse_subcommand(command: &str) -> Option<Mode> {
    match command {
